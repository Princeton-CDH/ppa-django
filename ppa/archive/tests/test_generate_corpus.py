...
def patched_solr_queryset(mock_solr_queryset):
    ...
    with patch(
        "ppa.archive.management.commands.generate_corpus.SolrQuerySet", new=mock_qs
...
def test_dictionary(tmpdir, patched_solr_queryset):
    call_command("generate_corpus", "--path", tmpdir.dirpath(), "--dictionary-as-text")
...
def test_metadata_file(tmpdir, patched_solr_queryset):
    call_command(
        "generate_corpus", "--path", tmpdir.dirpath(), "--preprocess", "strip_short"
    )
...
def test_invalid_preprocess_flags(tmpdir, patched_solr_queryset):
    ...
    with pytest.raises(CommandError):
        call_command(
            "generate_corpus", "--path", tmpdir.dirpath(), "--preprocess", "upper"
        )
