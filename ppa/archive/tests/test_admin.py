...
class TestDigitizedWorkAdmin(TestCase):
    ...
    def test_source_link(self):
        ...
        digwork = DigitizedWork.objects.create(
            source_id="njp.32101013082597", source_url=fake_url
        )
    ...
    def test_readonly_fields(self):
        ...
        assert set(digadmin.get_readonly_fields(Mock(), hathi_work)) == set(
            digadmin.readonly_fields + digadmin.hathi_readonly_fields
        )
    ...
    def test_add_works_to_collection(self):
        ...
        assert set(fakerequest.session["collection-add-ids"]) == set(
    ...
    def test_suppress_works(self, mock_solrclient):
        ...
        with patch.object(digworkadmin, "message_user") as mock_message_user:
            ...
            assert DigitizedWork.objects.filter(
                status=DigitizedWork.SUPPRESSED
            ).count() == len(all_ids)
