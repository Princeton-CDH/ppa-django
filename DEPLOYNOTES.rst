...
* This release updates Gale image url logic to rely on image URLs from the API, which are (or will be) encrypted and rotated periodically. Gale images should be reindexed via cron job using `python manage.py  --source Gale`.
...
* Gale/ECCO MARC records must be made available for import
  by splitting out binary MARC into a local pairtree storage::
    ...
    python manage.py split_marc ECCO1a-test_location_id.mrc ECCO1b-test_location_id.mrc ECCO2-test_location_id.mrc
